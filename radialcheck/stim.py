...
if __name__ == "__main__":
    """
    Example of how to use the RadialCheckerboard class
    """
    ...
    win = visual.Window(
        ...
        units='pix',
        color=[0, 0, 0],
        waitBlanking=True  # Wait for vertical blank
    )
    
    # Measure the actual frame rate
    print("Measuring frame rate...")
    actual_fps = win.getActualFrameRate(nIdentical=60, nMaxFrames=100, nWarmUpFrames=10, threshold=1)
    if actual_fps is not None:
        print(f"Measured frame rate: {actual_fps:.2f} Hz")
    else:
        print("Could not measure frame rate, using 60 Hz default")
        actual_fps = 60.0
    
    # Create a mock session object
    class Session:
        def __init__(self, win, monitor):
            self.win = win
            self.monitor = monitor
    
    session = Session(win, mon)
    
    # Example 1: Simple binary phase reversal (2 phases)
    print("\n=== Example 1: Binary Phase Reversal (2 phases) ===")
    radial_stim = RadialCheckerboard(
        session,
        n_wedges=16,
        n_rings=8,
        radius_deg=10,
        flicker_frequency=2,        # 2 Hz reversal
        animation_type='reversing',
        n_phases=2                   # Just black/white flip
    )
    
    clock = core.Clock()
    duration = 3
    print("Simple on/off reversal for 3 seconds...")
    while clock.getTime() < duration:
        radial_stim.draw(time=clock.getTime())
        win.flip()
    
    # Example 2: Smooth phase reversal (8 phases)
    print("\n=== Example 2: Smooth Phase Reversal (8 phases) ===")
    radial_stim.set_animation_type('reversing', n_phases=8)
    
    clock.reset()
    print("Smooth reversal with fade for 3 seconds...")
    while clock.getTime() < duration:
        radial_stim.draw(time=clock.getTime())
        win.flip()
    ...
    print("\n=== Example 3: Rotating Animation ===")
    radial_stim.set_animation_type('moving', n_phases=8)
    
    clock.reset()
    print("Rotating checkerboard for 3 seconds...")
    while clock.getTime() < duration:
        radial_stim.draw(time=clock.getTime())
        win.flip()
    
    # Example 4: Expanding/contracting rings
    print("\n=== Example 4: Radial Movement (Expanding) ===")
    radial_stim.set_animation_type('radial_moving', n_phases=8)
    
    clock.reset()
    print("Expanding/contracting rings for 3 seconds...")
    while clock.getTime() < duration:
        radial_stim.draw(time=clock.getTime())
        win.flip()
    
    # Example 5: Static examples with different phases
    print("\n=== Example 5: Static Patterns ===")
    print("Showing static examples with different phases...")
    for angular_phase in [0, np.pi/4, np.pi/2]:
        radial_stim.draw_static(
