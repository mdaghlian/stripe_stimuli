...
class GRATETrial(Trial):

    def __init__(self, session, trial_nr, spatial_frequency, speed_deg_per_sec, radius_deg, orientation, *args, **kwargs):
        ...
        if self.session.settings['stim']['scanner_sync']==True:
            ...
            phase_durations = [self.session.settings['stim']['trial_length']] 
    ...
    def get_events(self):
        ...
        if events:
            if 'q' in [ev[0] for ev in events]:  # specific key in settings?
                ...
                np.save(opj(self.session.output_dir, self.session.output_str+'_simple_response_data.npy'), {"Expected number of responses":len(self.session.dot_switch_color_times),
                                                                                  ...
                                                                                  f"Correct responses (within {self.session.settings['task_settings']['response interval']}s of dot color change)":self.session.correct_responses})
           
                if self.session.settings['stim']['Screenshot']==True:
            ...
            for key, t in events:
                ...
                if key == self.session.mri_trigger:
                    ...
                    if self.session.settings['stim']['scanner_sync']==True and t>0.1:                       
        ...
        if self.session.dot_count < len(self.session.dot_switch_color_times): 
            if self.session.clock.getTime() > self.session.dot_switch_color_times[self.session.dot_count] + \
                float(self.session.settings['task_settings']['response interval'])+0.1: #to give time to respond
