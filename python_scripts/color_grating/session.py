...
class GRATESession(PylinkEyetrackerSession):
    ...
    def __init__(self, output_str, output_dir, settings_file, eyetracker_on=False, **kwargs):
        ...
        self.session_num = kwargs.get('session_num', None)
        # Calculate total screen width in degrees of visual angle
        self.screen_width_deg = tools.monitorunittools.pix2deg(self.win.size[1], self.monitor)
        self.screen_height_deg = tools.monitorunittools.pix2deg(self.win.size[0], self.monitor)
        if self.settings['operating system'] == 'mac':
            self.screen_width_deg /= 2
            self.screen_height_deg /= 2
    ...
    def create_stimuli(self):
        ...
        col_list = self.settings['stim_settings']['trial_col']
        col_list = [i for i in col_list if i!="none"]     
        #generate PRF stimulus
        self.grate_stim = DriftingGratings(
            session=self, 
            orientations=ori_list,
            subject=self.subject, session_num=self.session_num,
            cols=col_list,
            ...
            screen_height_deg=self.screen_height_deg,
            screen_width_deg=self.screen_width_deg, 
            )    



        # ***** Stimuli for the fixation task *****
        #currently unused
    ...
    def create_trials(self):
        ...
        # colour 
        self.trial_col_full = []
        self.trial_col_full.extend(
            ['none']*self.settings['stim_settings']['start_blanks']
        )
        for i in self.settings['stim_settings']['trial_col']:
            if i == 'none':
                self.trial_col_full.extend(
                    ['none'] * self.settings['stim_settings']['off_time']
                )
            else:
                self.trial_col_full.extend(
                    [i] * self.settings['stim_settings']['on_time']
        ...
        self.trial_col_full.extend(
            ['none']*self.settings['stim_settings']['end_blanks']
        )
